...
export interface SkinAnalysisResult {
  success: boolean;
  message: string;
  mainCondition?: SkinCondition;
  otherConditions?: SkinCondition[];
}
 ...
 */
export async function analyzeSkinDirectly(file: File): Promise<SkinAnalysisResult> {
  try {
 ...
 */
export async function analyzeSkinViaBackend(file: File): Promise<SkinAnalysisResult> {
  try {
 ...
 */
function processPredictions(responseData: any): SkinCondition[] {
  ...
  if (responseData.predictions && Array.isArray(responseData.predictions)) {
    ...
    responseData.predicted_classes.forEach((className: string, index: number) => {
    ...
    Object.keys(responseData).forEach(key => {
      ...
      if (!isMetadataField(key) && typeof value === 'number') {
        conditions.push({
          name: key,
          confidence: value,
          confidencePercentage: Math.round(value * 100)
        });
      }
    });
  }
...
export function useSkinAnalysis() {
  ...
  const [result, setResult] = React.useState<SkinAnalysisResult | null>(null);
  const [error, setError] = React.useState<string | null>(null);

  const analyze = async (file: File, useBackend: boolean = true) => {
    setIsAnalyzing(true);
    setError(null);
    setResult(null);
