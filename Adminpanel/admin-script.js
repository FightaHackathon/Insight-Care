...
class AdminPanel {
    ...
    setupModalControls() {
        ...
        addClinicBtn?.addEventListener('click', () => this.openClinicModal());
        clinicModalClose?.addEventListener('click', () => this.closeModal('clinic-modal'));
        clinicCancelBtn?.addEventListener('click', () => this.closeModal('clinic-modal'));
    ...
    saveClinic() {
        const clinicData = {
            ...
            openingHours: document.getElementById('clinic-opening-hours').value,
            status: document.getElementById('clinic-status').value,
            roomCount: this.rooms.filter(r => r.clinicId === (this.currentEditId || this.generateId())).length
        };

        if (this.currentEditId) {
            const index = this.clinics.findIndex(c => c.id === this.currentEditId);
    ...
    renderUsers() {
        ...
        tbody.innerHTML = this.users.map(user => `
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        <button class="action-btn edit" onclick="adminPanel.openUserModal('${user.id}')" title="Edit">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" stroke="currentColor" stroke-width="2"/>
                                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" stroke-width="2"/>
                            </svg>
    ...
    renderClinics() {
        ...
        tbody.innerHTML = this.clinics.map(clinic => `
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        <button class="action-btn edit" onclick="adminPanel.openClinicModal('${clinic.id}')" title="Edit">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" stroke="currentColor" stroke-width="2"/>
                                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" stroke-width="2"/>
                            </svg>
                        </button>
                        <button class="action-btn delete" onclick="adminPanel.deleteClinic('${clinic.id}')" title="Delete">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                                <polyline points="3,6 5,6 21,6" stroke="currentColor" stroke-width="2"/>
                                <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2 2h4a2 2 0 0 1 2 2v2" stroke="currentColor" stroke-width="2"/>
                            </svg>
...
document.addEventListener('DOMContentLoaded', () => {
    adminPanel = new AdminPanel();
>>>>>>> fdd2716094e5f165e7299e6ac2c101c9abd6682a
});